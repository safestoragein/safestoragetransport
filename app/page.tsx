...
export default async function Home({ searchParams }: { searchParams: Promise<{ src?: string; date?: string; view?: string }> }) {
  ...
  // Escalations — post-completion issues raised from the Feedback page, worked to resolution
  if (view === "escalations") return <EscalationsBoard user={user} />;
  ...
  // Retrieval → Calls (telephony)
  if (view === "calls") return <CallsBoard user={user} />;
