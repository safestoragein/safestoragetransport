...
export interface StepTop { ref?: string; name?: string; phone?: string; booked?: string; area?: string }
          ...
          return (
            <Fragment key={s.label + i}>
              <div className="flex w-32 flex-col items-center px-1 text-center">
                ...
                <div className="mb-1 flex h-20 flex-col items-center justify-end leading-tight">
                  ...
                  {s.top?.area && <div className="max-w-[7.5rem] truncate text-[11px] font-medium text-slate-500" title={s.top.area}>📍 {s.top.area}</div>}
              ...
              {i < n - 1 && <div className={`h-1 w-10 shrink-0 rounded ${done ? "bg-emerald-400" : "bg-slate-200"}`} style={{ marginTop: "146px" }} />}
            </Fragment>
          );
        })}
      </div>
