...
export function proxy(request: NextRequest) {
  ...
  if ((pathname.startsWith("/api/schedule/generate") || pathname.startsWith("/api/schedule/diff") || pathname.startsWith("/api/schedule/rate-sync") || pathname.startsWith("/api/retrieval/sync")) && request.headers.get("x-vercel-cron")) return NextResponse.next();
