...
export function optimize(date: string, city: string, bookings: Booking[], vendors: Vendor[]): OptimizationResult {
  ...
  // can absorb that order, move it there — allowing a one-off 4th stop as an exception (the normal
  // cap stays 3). Only merges when a host is genuinely near, so it never creates a long detour.
  ...
  for (const v of vendors) {
    ...
    for (const h of vendors) {
      ...
      if (palletsOf(hbs) + b.pallets > h.maxPalletsPerDay + EPS) continue;
